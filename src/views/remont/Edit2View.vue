<template>
  <div class="container">
    <h1 align="center" class="m-3">Изменение ремонта {{ product.id }}</h1>
    <form class="row g-3">
        <div class="col-md-12">
        ...
        <div class="col-md-12">
          <label class="form-label">Время ремонта</label>
          <input v-model="product.vremyaremont" type="text" class="form-control" placeholder="Время ремонта"/>
        </div>
        <div align="center">        
...
export default {
  ...
  methods: { 
    ...
    async editproduct() {      
      await fetch("http://localhost:3000/remont/edit", {
        method: "POST",
        headers: {
          accept: "application/json",
          "content-type": "application/json",          
      ...
      await this.$router.push('/RemontView')
    },    
  },
